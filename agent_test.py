...
from utils_fusion import fusion,fusion_virtual
import cv2
from action import take_action, action_size,take_action_fusion,take_action_raw_fusion
...
class Agent:
    def __init__(self, prefix, data_dir):
        ...
        self.deep_feature_model_path = "./vgg16.npy"
        ...
        if self.use_features:
            if self.use_deep_feature:
                if self.use_color_feature:
                    # color_features = np.concatenate((self.get_hist(raw_data), self.get_hist(vit_raw)), axis=1)
                    color_features = self.get_hist(vit_raw)
                    deep_features = self.prep.get_feature(vit_raw)
                    features = []

                    # deep_features = np.concatenate((self.prep.get_feature(raw_data), self.prep.get_feature(vit_raw)), axis=1)
                    for i in range(self.batch_size):
                        if self.use_history:
    ...
    def test(self, in_order=False, idx=-1):
        test_result_dir = "test/" + self.prefix + "/results/"
        ...
        for i in range(state_raw.shape[0]):
            ...
            raw_vit[i] = raw_vit[i][:, :, (2, 1, 0)]
            raw_vit_retouched = Image.fromarray(
                np.uint8(np.clip((raw_vit[i] + 0.5) * 255, 0, 255)))
            raw_image_retouched.save(
                os.path.join(raw_dir, "%s_retouched.png" % fn[i]))
        return retouched_raw_images
    ...
    with tf.Session(config=config) as sess:
        ...
        if model_path:
            ...
            for i in range(agent.test_img_count):
                res_imgs = agent.test(in_order=True, idx=i)
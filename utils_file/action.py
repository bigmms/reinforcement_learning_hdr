action_size = 6
from utils_file.action_set import *
#from action_set_tf import *
from utils_file.utils_fusion import three_fusion
def PIL2array(img):
...
def take_action_fusion(image, action_idx, virtual):
    image_np_u = (image[0:round(image.shape[0] / 2), :, :] + 0.5) * 255
    image_np_o = (image[round(image.shape[0] / 2):, :, :] + 0.5) * 255

    return_np = None
    if action_idx == 0:
        virtual = contrast(virtual+ 0.5, 0.95)
        return_np = three_fusion(image_np_u, image_np_o, virtual*255)

    if action_idx == 1:
        virtual = contrast(virtual+ 0.5, 1.05)
        return_np = three_fusion(image_np_u, image_np_o, virtual*255)

    if action_idx == 2:
        virtual = color_saturation(virtual+ 0.5, 0.95)
        return_np = three_fusion(image_np_u, image_np_o, virtual*255)

    if action_idx == 3:
        virtual = color_saturation(virtual+ 0.5, 1.05)
        return_np = three_fusion(image_np_u, image_np_o, virtual*255)

    if action_idx == 4:
        virtual = brightness(virtual + 0.5, 0.93)
        return_np = three_fusion(image_np_u, image_np_o, virtual*255)

    if action_idx == 5:
        virtual = brightness(virtual+ 0.5, 1.07)
        return_np = three_fusion(image_np_u, image_np_o, virtual*255)
    return return_np - 0.5, virtual-0.5

def take_action_raw_fusion(virtual, raw_images_rawsize, action_idx):

    image_np_u = (raw_images_rawsize[0:round(raw_images_rawsize.shape[0] / 2), :, :] + 0.5) * 255
    image_np_o = (raw_images_rawsize[round(raw_images_rawsize.shape[0] / 2):, :, :] + 0.5) * 255

...
def contrast(image_rgb, b):
    ...
    image_v = image_hsv[:, :, 2]
    mean = np.mean(image_v)
    degenerate = np.zeros(image_v.shape) + mean
    image_hsv[:, :, 2] = b * image_v + (1 - b) * degenerate
...
def brightness(image_rgb, b):
    image_hsv = rgb2hsv(image_rgb)
    image_v = image_hsv[:, :, 2]
    degenerate = np.zeros(image_v.shape)
    image_hsv[:, :, 2] = b * image_v + (1 - b) * degenerate
    image_rgb = hsv2rgb(image_hsv)
    image_rgb = np.clip(image_rgb, 0, 1)
    return image_rgb
...
def color_saturation(image_rgb, b):
    ...
    degenerate = image_ycbcr[:, :, 1:3].mean(axis=2)
    image_ycbcr[:, :, 1] = b * image_cb + (1 - b) * degenerate
    image_ycbcr[:, :, 2] = b * image_cr + (1 - b) * degenerate
